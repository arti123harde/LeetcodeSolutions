...
public class EditDistance {
    ...
    public static int minDistance(String word1, String word2) {
        ...
        for (int word1Index = 1; word1Index <= word1Length; word1Index++) dp[word1Index][0] = word1Index;
        for (int word2Index = 1; word2Index <= word2Length; word2Index++) dp[0][word2Index] = word2Index;
        for (int word1Index = 1; word1Index <= word1Length; word1Index++) {
            for (int word2Index = 1; word2Index <= word2Length; word2Index++) {
                if (word2.charAt(word2Index - 1) == word1.charAt(word1Index - 1))
                    ...
                    int min1 = Math.min(dp[word1Index][word2Index - 1], dp[word1Index - 1][word2Index - 1]);
                    dp[word1Index][word2Index] = Math.min(dp[word1Index - 1][word2Index], min1) + 1;
                }
            }
        }
